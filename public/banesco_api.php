...
class BanescoAPI {
    private static function getToken() {
        ...
        curl_close($ch);
        if ($httpCode !== 200 || !$response) throw new Exception("Error obteniendo token Banesco SSO. Code: $httpCode. Response: $response");
        $tokenData = json_decode($response, true);
        if (!isset($tokenData['access_token'])) throw new Exception('Token de Banesco no encontrado en la respuesta.');
        $expiresIn = $tokenData['expires_in'] ?? 300;
        file_put_contents(BANESCO_TOKEN_CACHE, json_encode(['access_token'=>$tokenData['access_token'],'expires_at'=>time()+$expiresIn-60]));
    ...
    public static function checkTransaction($paymentId, $customerIdR = 'J402638850') {
        ...
        $error = curl_error($ch);
        curl_close($ch);
        if ($error) throw new Exception("Error cURL Banesco API: $error");
        $data = json_decode($response, true);
        $httpStatus = $data['httpStatus'] ?? null;
        if (!$httpStatus) throw new Exception("Respuesta inválida de API Banesco. Code: $httpCode.");
        if ($httpStatus['statusCode'] === '200' && $httpStatus['statusDesc'] === 'OK') return ['success'=>true,'data'=>$data['dataResponse'] ?? null];
        if ($httpStatus['statusCode'] === '70001') return ['success'=>false,'message'=>'No se encontraron resultados para la referencia indicada. Verifique que la transferencia a Banesco sea exitosa.'];
        if ($httpStatus['statusCode'] === 'CRT503') throw new Exception('Banesco: Servicio en horario de mantenimiento. Intente más tarde.');
        return ['success'=>false,'message'=>$httpStatus['statusDesc'] ?? "Error en la validación del banco (Code: {$httpStatus['statusCode']})"];
    }
}
?>
